...
class PCBPCompile:
    ...
    def __init__(self, pathFile):
        ...
        self.pathPCBPBase = os.path.basename(pathFile)
        self.pathCBPBase = self.pathPCBPBase[1:-4] + '.txt'
    ...
    def generalCompile(self, dirCBP, dirPCBP, svr, lenNum, CBPOutput):
        ...
        print('Parsing: {}'.format(self.pathPCBPBase))
        ...
        PCBPImportNum = 1
        PCBPImportStart = True
        
        # Detects all PCBPIMPORT commands
        while PCBPImportNum:
            if PCBPImportStart == False:
                CBPSplitList = CBPImportList
                CBPImportList = []
            PCBPImportNum = 0
            PCBPImportStart = False
                ...
                if s[:10] == 'PCBPIMPORT':
                    ...
                    try:
                        ...
                        if CBPOpen:
                            ...
                            for s2 in CBPImportCalcList:
                                CBPImportList.append(s2)
                            CBPOpen.close()
                else:
        ...
        if CBPOutput == False:
            subprocess.call(['java', '-jar', 'CBP.jar', self.directoryCBP, '-print', '-w', '6', '-c', 'Number', '-noreset', '-minbuild', '50', '-notrack'], stdout = FileNull)
        else:
            subprocess.call(['java', '-jar', 'CBP.jar', self.directoryCBP, '-print', '-w', '6', '-c', 'Number', '-noreset', '-minbuild', '50', '-notrack'])
